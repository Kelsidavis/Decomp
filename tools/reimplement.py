...
def call_llm(endpoint, model, name, sig, meta, max_tokens=256, timeout=120):
    prompt = (
        "Re-implement this C function based on metadata.\n"
        f"Name: {name}\n"
        f"Signature: {sig}\n"
        ...
        for m in FUNC_RE.finditer(code):
            ...
            if name in labels:
                try:
                    ...
                    # naive insertion: replace the matched header+{ with header + body + }
                    header = m.group(0)  # includes opening brace
                    replacement = f"{sig} {{\n{body}\n}}"
                    new_code = new_code.replace(header, replacement, 1)
                except Exception as e:
                    # leave function untouched on failure
                    pass
